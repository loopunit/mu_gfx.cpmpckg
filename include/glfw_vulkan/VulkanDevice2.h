...
namespace FGC
{
	...
	class VulkanDevice2 : public VulkanDeviceFn
	{
		// types
	public:
...
// VkPhysicalDeviceVulkan12Properties				properties120;
	...
	protected:
		...
		// methods
	public:
		struct window_specific
		{
			VkSurfaceKHR _vkSurface;

			bool CreateInstance(UniquePtr<IVulkanSurface>& surface, VkInstance _vkInstance)
			{
			...
			void DestroyInstance(VkInstance _vkInstance)
			{
				if (_vkSurface)
				{
			...
			ND_ VkSurfaceKHR GetVkSurface() const
			{
				return _vkSurface;
			}
		};
		...
		ND_ EnabledFeatures const& GetFeatures() const
		{
			return _features;
		}
		ND_ DeviceProperties const& GetProperties() const
		{
			return _properties;
		}

		ND_ VkDevice GetVkDevice() const
		{
			return _vkLogicalDevice;
		}
		ND_ VkPhysicalDevice GetVkPhysicalDevice() const
		{
			return _vkPhysicalDevice;
		}
		ND_ VkInstance GetVkInstance() const
		{
			return _vkInstance;
		}
		ND_ InstanceVersion GetVkVersion() const
		{
			return _vkVersion;
		}
		ND_ ArrayView<VQueue> GetVkQueues() const
		{
			return _vkQueues;
		}
		ND_ VQueuePtr GetQueue(VQueueType type) const
		{
			return uint(type) < _queueTypes.size() ? _queueTypes[uint(type)] : null;
		}
	...
	class VulkanDevice2Initializer final : public VulkanDevice2
	{
		// types
	public:
	...
	private:
		...
		// methods
	public:
		VulkanDevice2Initializer();
		~VulkanDevice2Initializer();

		ND_ InstanceVersion GetInstanceVersion() const;

		bool CreateInstance(
			NtStringView appName, NtStringView engineName, ArrayView<const char*> instanceLayers, ArrayView<const char*> instanceExtensions = {}, InstanceVersion version = {1, 2},
			uint appVer = 0, uint engineVer = 0);
		...
		bool CreateLogicalDevice(ArrayView<QueueCreateInfo> queues, VkBool32 has_surface, VkBool32 can_present, ArrayView<const char*> extensions = {});
		bool SetLogicalDevice(VkDevice value, VkBool32 has_surface, VkBool32 can_present, ArrayView<const char*> extensions = {});
	...
	private:
		...
		bool _SetupQueues(ArrayView<QueueCreateInfo> queue, VkBool32 has_surface, VkBool32 can_present);
		...
		bool _SetupQueueTypes(VkBool32 has_surface, VkBool32 can_present);
		bool _AddGraphicsQueue(VkBool32 has_surface, VkBool32 can_present);
		bool _AddAsyncComputeQueue();
		bool _AddAsyncTransferQueue();

		VKAPI_ATTR static VkBool32 VKAPI_CALL _DebugUtilsCallback(
